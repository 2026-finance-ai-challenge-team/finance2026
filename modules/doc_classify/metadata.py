...
def extract_extracted_metadata(
    extracted: Any,
    *,
    signatures: list[dict[str, Any]] | None = None,
    expected_owner_name: str | None = None,
    ...
    """Reuse text/OCR already produced by ``classify_files``.

    This avoids a second CLOVA request. When an OCR response exists in the
    request-scoped cache, its coordinates are preserved as evidence block IDs;
    otherwise embedded PDF text is converted to the same internal shape.
    """
    from .extract import _cache_path

    cached = _cache_path(extracted.path, cache_dir)
    if any(page.method == "ocr" for page in extracted.pages) and cached.is_file():
