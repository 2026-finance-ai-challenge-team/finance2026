"""`scenarios.csv` 생성 — 필수 서류를 하나씩 빼면서 "부족" 출력 여부를 보는 표.

필수 목록은 규칙 DB에서 조회하는 것이 원칙. 실패 시 하드코딩으로 폴백하되
폴백 사실을 반드시 출력. 조용히 폴백하면 DB 변경을 모른 채 계속 통과.

    python tools/make_scenarios.py
    EVAL_DB_URL=postgresql://... python tools/make_scenarios.py
"""
...
# TODO(swap): DB 조회 연결 시 아래 두 블록 삭제. 현재는 하나은행 법인계좌 개설 안내를
# 사람이 옮겨적은 임시값 — Q1(목록이 맞나) 미검증
FALLBACK_REQUIRED = [
...
def load_requirements() -> tuple[list[str], dict[str, list[str]], bool]:
    ...
    try:
        ...
        print("psycopg2 없음 — DB 조회 생략", file=sys.stderr)
        return FALLBACK_REQUIRED, FALLBACK_CHOICE_GROUPS, False
    ...
    try:
        with psycopg2.connect(db_url) as conn, conn.cursor() as cur:
            ...
            rows = cur.fetchall()
    except Exception as exc:  # 연결 실패도 폴백 사유로 보고
        print(f"DB 조회 실패 — {exc}", file=sys.stderr)
        return FALLBACK_REQUIRED, FALLBACK_CHOICE_GROUPS, False

    if not rows:
        print(f"DB에 {CHANNEL}/{VISITOR_TYPE} 요건 없음", file=sys.stderr)
        return FALLBACK_REQUIRED, FALLBACK_CHOICE_GROUPS, False
...
def canonical_files() -> dict[str, str]:
    """doc_type -> 대표 샘플 파일. 기한만료본·이미지 변형 제외."""
...
def build_rows(required: list[str], groups: dict[str, list[str]], files: dict[str, str]) -> list[dict]:
    ...
    # 기준 묶음 = 필수 전부 + 각 선택 그룹에서 하나. 이 상태에서 부족 출력은 오답
    ...
    # 하나씩 빼기. 뺐는데 부족으로 안 나오면 그 서류를 실제로는 미확인
    ...
    traps = [
        ...
        ("t3", "대체", required, group_id, "", "대체서류 둘 다 없음 — 부족 출력 대상"),
        ("t4", "대체", required + [first, second], "", "", "둘 다 냈다고 둘 다 요구하면 과잉"),
        ("t6", "기한", None, "", "corporate_registry_certificate", "1년 지난 등기부 — 기한 만료"),
        ("t7", "엣지", ["__취업후기__"], "|".join(all_expected), "", "증명서 아닌 파일 하나 — 전부 부족, 안 죽음"),
        ("t8", "엣지", [], "|".join(all_expected), "", "파일 0개 — 전부 부족, 안 죽음"),
        ("t9", "유사쌍", ["__사업자등록증명__"], "|".join(all_expected), "", "사업자등록증명을 사업자등록증으로 오인 금지"),
        ("t10", "유사쌍", ["__개인인감__"], "|".join(all_expected), "", "개인인감증명서를 법인인감증명서로 오인 금지"),
    ]
    ...
    # t5는 같은 파일 2회 투입. 중복 제거 여부가 초점이라 별도 생성
...
def main() -> int:
    ...
    if not LABELS_CSV.is_file():
        print("labels.csv 없음 — generate.py 먼저 실행", file=sys.stderr)
        return 1
    ...
    if from_db:
        ...
        print("요건 출처: 하드코딩 폴백 — DB 미연결. 규칙 변경 미반영")
    print(f"  필수 {len(required)}종 · 선택 그룹 {len(groups)}개")

    files = canonical_files()
    unmapped = [d for d in required if d not in files]
    if unmapped:
        print(f"  샘플 없는 필수 서류 {unmapped} — generate.py 목록 확인", file=sys.stderr)
    ...
    print(f"scenarios.csv {len(rows)}행 (자동 {auto} · 함정 {len(rows) - auto})")
    if args.print:
        for row in rows:
